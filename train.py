...
def main(batch_size,ep):
    ...
    #? All the code below this in this def should be fine
    ...
    np.random.seed(None)
    num_val = 10#int(len(lines)*val_split)
    ...
    if True:
        ...
        #batch_size = 10#32 # note that more GPU memory is required after unfreezing the body
        print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
        model.fit_generator(data_generator_wrapper(lines[:num_train], batch_size, input_shape, anchors, num_classes),
            steps_per_epoch=max(1, num_train//batch_size),
            validation_data=data_generator_wrapper(lines[num_train:], batch_size, input_shape, anchors, num_classes),
...
def data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes):
    ...
    while True:
        ...
        box_data = np.array(box_data)#?box_data should include the class label
        y_true = preprocess_true_boxes(box_data, input_shape, anchors, num_classes)#? Handles class label
